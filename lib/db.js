...
function Db(url, conf, dir, prefix) {

  var couch = nano(url);

  if (_.isString(prefix)) {
    _.keys(conf).forEach(function(originalName) {
      conf[prefix + originalName] = conf[originalName];
      delete conf[originalName];
    });
  }

  function _databases(errorCb, successCb) {
  ...
  function setUpDatabases(cb) {
    ...
    _databases(cb, function (exist, inexist) {
