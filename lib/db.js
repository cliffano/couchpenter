  ...
  _.keys(dbSetup).forEach(function (dbName) {
    dbSetup[dbName].forEach(function (doc) {
      if (doc._id && doc._id.indexOf('_design/') === 0 && doc.views) {
        function task(cb) {
          cb = self._handle(cb, { dbName: dbName, docId: doc._id, 
                                  message: 'warmed ' + _.keys(doc.views).length + ' views' });
          async.each(_.keys(doc.views), function (viewName, cb) {
            self.couch.use(dbName).view(doc._id.replace('_design/', ''), viewName, { limit: 1 }, cb);
          }, cb);
        } 
        tasks.push(task);
      }
