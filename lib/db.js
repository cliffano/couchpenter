...
function Db(url, conf, dir) {
      ...
      tasks[db] = function (cb) {
        ...
        couch.use(db).fetch({ keys: _.pluck(docs, '_id') }, {}, function (err, result) {
          var existingDocs = _rows2Map(result.rows),
            toProcess = fn(db, docs, existingDocs);
  ...
  function tearDownDocuments(cb) {
    ...
    _documents(function (db, docs, existingDocs) {
      ...
      // set delete to true for all existing docs, remove inexisting docs
      for (var i = 0, ln = docs.length; i < ln; i += 1) {
        if (existingDocs[docs[i]._id]) {
          docs[i]._rev = existingDocs[docs[i]._id]._rev;
          docs[i]._deleted = true;
        } else {
          console.log('- ignoring inexisting document: ' + docs[i]._id + '; in database: ' + db);
          docs.splice(i, 1);
        }
      }

      if (!_.isEmpty(docs)) {
        console.log('- deleting documents: ' + _.pluck(docs, '_id') + '; in database: ' + db);
      }
