 ...
 * @param {Object} configFile: JSON file containing Couchpenter data config
...
function Couchpenter(url, configFile, opts) {
  ...
  this.dbConfig = (configFile) ?
    JSON.parse(bag.cli.readCustomConfigFileSync(configFile || 'couchpenter.json'))
    : {};
  ...
  if (this.opts.prefix) {
    _.keys(this.dbConfig).forEach(function (dbName) {
      this.dbConfig[this.opts.prefix + dbName] = this.dbConfig[dbName];
  ...
  taskNames.forEach(function (taskName) {
    tasks.push(function (cb) {
      var data = (taskName.match(/Databases$/)) ? 
        ...
        _docs(self.dbConfig, self.opts.dir);
